...
<xsl:template match="/">
      ...
      <svrl:schematron-output xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
        ...
        Alternativement, si l'attribut nullFlavor est présent, il doit avoir l'une des valeurs admises par le CI-SIS. 
         ...
         <svrl:active-pattern>
            <xsl:attribute name="id">JDV_substanceAdministration_RouteOfAdministration</xsl:attribute>
            <svrl:text>Conformité d'un élément codé obligatoire par rapport à un jeu de valeurs du CI-SIS</svrl:text>
            <xsl:apply-templates/>
         </svrl:active-pattern>
...
<xsl:choose>
         ...
         <xsl:otherwise>
		    ...
		    <!--ASSERT -->
<xsl:choose>
         <xsl:when test="(             (@xsi:type and not(@xsi:type = 'CD') and not(@xsi:type = 'CE')) or              (document($jdv_typeCode_1)//svs:Concept[@code=$att_code and @codeSystem=$att_codeSystem])             or  (document($jdv_typeCode_2)//svs:Concept[@code=$att_code and @codeSystem=$att_codeSystem])             or  (document($jdv_typeCode_3)//svs:Concept[@code=$att_code and @codeSystem=$att_codeSystem]))"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
            ...
            doit faire partie du jeu de valeurs <xsl:text/>
                  ...
                  <xsl:text/> ou <xsl:text/>
                  <xsl:value-of select="$jdv_typeCode_3"/>
                  <xsl:text/> .
        </svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
...
<xsl:choose>
   ...
   <!--PATTERN JDV_substanceAdministration_RouteOfAdministration-->


	<!--RULE -->
<xsl:template match="cda:substanceAdministration[cda:templateId/@root='1.3.6.1.4.1.19376.1.5.3.1.4.7' or cda:templateId/@root='1.2.250.1.213.1.1.3.13' or cda:templateId/@root='1.3.6.1.4.1.19376.1.5.3.1.1.13.3.2']/cda:routeCode"
                 priority="1000"
                 mode="M46">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                       context="cda:substanceAdministration[cda:templateId/@root='1.3.6.1.4.1.19376.1.5.3.1.4.7' or cda:templateId/@root='1.2.250.1.213.1.1.3.13' or cda:templateId/@root='1.3.6.1.4.1.19376.1.5.3.1.1.13.3.2']/cda:routeCode"/>
      <xsl:variable name="att_code" select="@code"/>
      <xsl:variable name="att_codeSystem" select="@codeSystem"/>
      <xsl:variable name="att_displayName" select="@displayName"/>
...
<xsl:choose>
         ...
         <xsl:otherwise>
		    ...
		    <!--ASSERT -->
<xsl:choose>
         <xsl:when test="(             @nullFlavor or             (@xsi:type and not(@xsi:type = 'CD') and not(@xsi:type = 'CE')) or              (document($jdv_substanceAdministration_RouteOfAdministration)//svs:Concept[@code=$att_code and @codeSystem=$att_codeSystem])             )"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
            ...
            [<xsl:text/>
                  ...
                  <xsl:text/>] 
            doit faire partie du jeu de valeurs <xsl:text/>
                  <xsl:value-of select="$jdv_substanceAdministration_RouteOfAdministration"/>
                  <xsl:text/>.
        </svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
...
<xsl:variable name="jdv_authenticatorSpecialty"
   ...
   <xsl:variable name="jdv_typeCode_1"
                 select="'../../jeuxDeValeurs/TRE_A04-LoincTypeDocument.xml'"/>
   <xsl:variable name="jdv_typeCode_2" select="'../../jeuxDeValeurs/TRE_A04-Loinc.xml'"/>
   <xsl:variable name="jdv_typeCode_3"
   ...
   <xsl:variable name="jdv_substanceAdministration_RouteOfAdministration"
